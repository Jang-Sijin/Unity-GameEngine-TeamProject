...
public class FpsController : MonoBehaviour
{
    ...
    // 다른 오브젝트 관리
    public GameObject active_F1_key;
    public GameObject active_ESCAPE_key;
    public bool Ui_F1_key = false;
    public bool Ui_ESCAPE_key = false;
    ...
    {
        // ESC를 누를 경우 출력
        if (Input.GetKeyDown(KeyCode.Escape) && !Ui_F1_key)
        {
            Ui_ESCAPE_key = !Ui_ESCAPE_key;
            active_ESCAPE_key.SetActive(Ui_ESCAPE_key);
        }
        else if (Input.GetKeyDown(KeyCode.F1) && !Ui_ESCAPE_key)
        {
            Ui_F1_key = !Ui_F1_key;
            active_F1_key.SetActive(Ui_F1_key);
        }


        if (!Ui_F1_key && !Ui_ESCAPE_key)
        {
